...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// Connection : 특정 DB와의 연결 정보를 저장한 객체
		// (== dbeaver에서 DB 연결을 위해 주소, 계정, 비밀번호 저장한 것과 같음)
		Connection conn = null;
		
		// Statement : SQL을 String 형태로 DB에 전달하고 결과를 받아오는 객체
		Statement stmt = null;
		
		// ResultSet : SELECT 결과를 저장하는 객체 (조회 결과는 0행 이상, 커서 이용해 1행씩 접근 가능)
		ResultSet rs = null;
		
		try {
			
			/* 2. DriveManager 객체를 이용해서 Connection 생성하기 */
			...
			// ** JDBC에서 SQL 작성 시 ;을 작성하면 안됨
			
			
			/* 4. Statement 객체 생성 */
			stmt = conn.createStatement();
			// Connection에 연결된 DB로 SQL 전달하고 결과를 받아오는 Statemenet 객체 생성
			...
			/* 6. 조회 결과가 담겨 있는 ResultSet을 커서를 이용해 1행씩 접근하며 
			 * 각 행의 컬럼 값 얻어오기 */
			
			while(rs.next()) {
				// rs.next() : 커서를 한 행씩 이동시켜 해당 행이 있으면 true, 없으면 false
				
				// rs.get(자료형(컬럼명 | 순서)
				// - 현재 접근한 행의 컬럼 값 얻어오기
				
				// [java]						[db]
				// String 					CHAR, VARCAHR2
			...
			// 추가한 라이브러리가 없거나, 클래스 경로를 잘못 작성하면 발생
			e.printStackTrace();
		} finally {
			
			/* 7. 사용 완료 된 JDBC 객체 자원 반환 
