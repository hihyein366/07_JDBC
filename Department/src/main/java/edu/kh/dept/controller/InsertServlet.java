...
public class InsertServlet extends HttpServlet{
	...
	// a태그, JS(location.href), 주소창 직접 작성, <form method="GET">
	// -> GET 방식 요청
	
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

		// JSP로 요청 위임해서 부서추가 화면 보여주기
		String path = "/WEB-INF/views/insert.jsp";
		
		// 요청 발송자(RequestDispatcher)를 이용해서 요청위임 하기
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// 클라이언트 접속 시 1개 생성, 브라우저 닫거나 만료 되었을 때 소멸
			...
			// -> redirect는 재요청 == request 객체가 다시 만들어진다(== 기존 request 사라짐)
